<!-- Header -->
<header class="header2" style="padding-top: 100px">
    <h1>PROGRAM & KEGIATAN</h1>
    <h2>RENSTRA</h2>
    <h3> @switch($title)
            @case('Dokumen Renstra 2017-2022')
                Dokumen Renstra 2017-2022
            @break

            @default
                Dokumen Renstra 2022-2027
        @endswitch
    </h3>
    <div class="divider2"></div>
</header>

<!-- Container -->
<div class="container2">
    <section class="document-section2">
        <h3>Dokumen Renstra 2017-2022</h3>

        <div class="document-item2">
            <h4>Renstra 2017-2022 Perubahan</h4>
            <p>Renstra 2017-2022 Perubahan</p>
            <div class="button-group2">
                <a href="#" class="btn2"><i class="fas fa-download"></i> Download</a>
                <a href="#" class="btn2"><i class="fas fa-book-open"></i> Lihat</a>
                <a href="#" class="btn2"><i class="fas fa-link"></i> Copy Link</a>
            </div>
        </div>

        <div class="document-item2">
            <h4>IKU Dinas Kesehatan DIY Tahun 2018-2022</h4>
            <p>IKU Dinas Kesehatan DIY Tahun 2018-2022</p>
            <div class="button-group2">
                <a href="#" class="btn2"><i class="fas fa-download"></i> Download</a>
                <a href="#" class="btn2"><i class="fas fa-book-open"></i> Lihat</a>
                <a href="#" class="btn2"><i class="fas fa-link"></i> Copy Link</a>
            </div>
        </div>

        <div class="document-item2">
            <h4>IKU Dinas Kesehatan DIY Tahun 2018-2022 Perubahan</h4>
            <p>IKU Dinas Kesehatan DIY Tahun 2018-2022 Perubahan</p>
            <div class="button-group2">
                <a href="#" class="btn2"><i class="fas fa-download"></i> Download</a>
                <a href="#" class="btn2"><i class="fas fa-book-open"></i> Lihat</a>
                <a href="#" class="btn2"><i class="fas fa-link"></i> Copy Link</a>
            </div>
        </div>
    </section>
</div>
